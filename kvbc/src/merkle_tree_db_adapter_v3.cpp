...
#include "merkle_tree_v3_block_family.h"
#include "Logger.hpp"
#include "sliver.hpp"
...
// Serialization code reuses the v2 definitions.
using namespace v2MerkleTree;
...
void createColumnFamilies(NativeClient *db) { BlockFamily::create(db); }
...
std::pair<Value, BlockId> DBAdapter::getValue(const Key &key, const BlockId &blockVersion) const {
  ...
  {
    Sliver block_key;
    if (blockVersion < getLastReachableBlockId()) {
      block_key = BlockFamily::serializeKey(blockVersion);
    } else {
      block_key = BlockFamily::serializeKey(getLastReachableBlockId());
    }
    auto block = rocksdb_->get(BlockFamily::NAME, block_key);
    if (!block) {
    ...
    }
    {
      ...
      stateRootVersion = detail::deserializeStateRootVersion(Sliver(std::move(block.value())));
    }
  }
...
BlockId DBAdapter::loadGenesisBlockId() const {
  auto iter = rocksdb_->getIterator(BlockFamily::NAME);
  iter.seekAtLeast(BlockFamily::serializeKey(INITIAL_GENESIS_BLOCK_ID));
  if (iter) {
    return BlockFamily::deserializeKey(iter.keyView().data());
  }
  return 0;
}
...
BlockId DBAdapter::loadLastReachableBlockId() const {
  ...
  // allowed block ID.
  auto iter = rocksdb_->getIterator(BlockFamily::NAME);
  iter.seekAtMost(BlockFamily::serializeKey(MAX_BLOCK_ID));
  if (iter) {
    auto block_id = iter.key();
    LOG_INFO(logger_, "Latest reachable block ID " << block_id);
    return BlockFamily::deserializeKey(block_id.data());
  }
...
RawBlock DBAdapter::getRawBlock(const BlockId &blockId) const {
  ...
  auto raw_block = rocksdb_->get(BlockFamily::NAME, BlockFamily::serializeKey(blockId));
  if (!raw_block.has_value()) {
    // A block node is not found. Look for a state transfer block.
    Sliver block;
  ...
  auto blockNodeSliver = Sliver(std::move(raw_block.value()));
...
BatchedInternalNode DBAdapter::Reader::get_latest_root() const {
  ...
  const auto block = Sliver(*adapter_.getNativeClient()->get(BlockFamily::NAME, BlockFamily::serializeKey(lastBlock)));
  const auto stateRootVersion = detail::deserializeStateRootVersion(block);
...
block::detail::Node DBAdapter::getBlockNode(BlockId blockId) const {
  auto block = rocksdb_->get(BlockFamily::NAME, BlockFamily::serializeKey(blockId));
  if (block) {
    return block::detail::parseNode(Sliver(std::move(block.value())));
  }
  const auto msg = "Failed to find block node for block ID = " + std::to_string(blockId);
  LOG_ERROR(logger_, msg);
  throw std::runtime_error{msg};
}
...
WriteBatch DBAdapter::lastReachableBlockKeyDeletes(BlockId blockId) const {
  ...
  for (const auto &key : provableKeys) {
    batch.del(DBKeyManipulator::genDataDbKey(key.first, blockNode.stateRootVersion));
  }

  // Delete non-provable keys.
  for (const auto &key : nonProvableKeys) {
    batch.del(DBKeyManipulator::genNonProvableDbKey(blockId, key.first));
  }

  // Delete internal keys at the last version.
  delInBatch(batch, internalProvableKeysForVersion(blockNode.stateRootVersion));

  // Delete the block node key.
  batch.del(BlockFamily::serializeKey(blockId));

  // Clear the tree stale index for the last version.
  delInBatch(batch, staleIndexProvableKeysForVersion(blockNode.stateRootVersion));

  // Clear the non-provable stale index for the last block
  delInBatch(batch, staleIndexNonProvableKeysForBlock(blockId));

  return batch;
}

WriteBatch DBAdapter::genesisBlockKeyDeletes(BlockId blockId) const {
  const auto blockNode = getBlockNode(blockId);
  ...
  for (const auto &staleKey : staleKeys) {
    batch.del(DBKeyManipulator::extractKeyFromProvableStaleKey(staleKey));
  }

  // Clear the stale index for the last version.
  delInBatch(batch, staleKeys);
  ...
  delInBatch(batch, nonProvableStaleKeys);

  // Delete stale keys.
  for (const auto &key : nonProvableStaleKeys) {
    batch.del(DBKeyManipulator::extractKeyFromNonProvableStaleKey(key));
  }
...
void DBAdapter::deleteKeysForBlock(WriteBatch &&batch, BlockId blockId) const {
...
bool DBAdapter::hasBlock(const BlockId &blockId) const {
  ...
  if (!rocksdb_->get(BlockFamily::NAME, BlockFamily::serializeKey(blockId))) {
