...
struct Recorders {
  ...
  static constexpr int64_t MAX_US = 1000 * 1000 * 60;         // 1m
  ...
  Recorders() {
    ...
    registrar.perf.registerComponent("sparse_merkle",
                                     {{"update", update},
                                      ...
                                      {"dba_add_block_multiput", dba_add_block_multiput},
                                      {"dba_get_value_seek_block_non_provable", dba_get_value_seek_block_non_provable},
                                      {"dba_get_value_seek_block", dba_get_value_seek_block}});
  }
  ...
  std::shared_ptr<Recorder> internal_node_update_hashes = std::make_shared<Recorder>(1, MAX_US, 3, Unit::MICROSECONDS);
  std::shared_ptr<Recorder> internal_node_insert = std::make_shared<Recorder>(1, MAX_US, 3, Unit::MICROSECONDS);
  std::shared_ptr<Recorder> internal_node_remove = std::make_shared<Recorder>(1, MAX_US, 3, Unit::MICROSECONDS);

  // Used in merkle_tree_db_adapter.cpp
  std::shared_ptr<Recorder> dba_batch_to_db_updates = std::make_shared<Recorder>(1, MAX_US, 3, Unit::MICROSECONDS);
  std::shared_ptr<Recorder> dba_get_value = std::make_shared<Recorder>(1, MAX_US, 3, Unit::MICROSECONDS);
  ...
  std::shared_ptr<Recorder> dba_link_st_chain = std::make_shared<Recorder>(1, MAX_US, 3, Unit::MICROSECONDS);
  std::shared_ptr<Recorder> dba_num_blocks_for_st_link = std::make_shared<Recorder>(1, 1000000, 3, Unit::COUNT);
  std::shared_ptr<Recorder> dba_add_raw_block = std::make_shared<Recorder>(1, MAX_US, 3, Unit::MICROSECONDS);
  std::shared_ptr<Recorder> dba_delete_block = std::make_shared<Recorder>(1, MAX_US, 3, Unit::MICROSECONDS);
  std::shared_ptr<Recorder> dba_get_leaf_key_val_at_most_version =
      std::make_shared<Recorder>(1, MAX_US, 3, Unit::MICROSECONDS);
  std::shared_ptr<Recorder> dba_delete_keys_for_block = std::make_shared<Recorder>(1, MAX_US, 3, Unit::MICROSECONDS);
  std::shared_ptr<Recorder> dba_has_block = std::make_shared<Recorder>(1, MAX_US, 3, Unit::MICROSECONDS);
  std::shared_ptr<Recorder> dba_keys_for_version = std::make_shared<Recorder>(1, MAX_US, 3, Unit::MICROSECONDS);
  ...
  std::shared_ptr<Recorder> dba_add_block_multiput =
      std::make_shared<Recorder>(1, 60 * 1000 * 1000, 3, Unit::MICROSECONDS);
  std::shared_ptr<Recorder> dba_get_value_seek_block_non_provable =
      std::make_shared<Recorder>(1, 60 * 1000 * 1000, 3, Unit::MICROSECONDS);
  std::shared_ptr<Recorder> dba_get_value_seek_block =
      std::make_shared<Recorder>(1, 60 * 1000 * 1000, 3, Unit::MICROSECONDS);
};
