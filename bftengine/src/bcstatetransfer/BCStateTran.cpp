...
void BCStateTran::createCheckpointOfCurrentState(uint64_t checkpointNumber) {
  ...
  {// txn scope
    ...
    metrics_.last_stored_checkpoint_.Get().Set(checkpointNumber);
  }
}
...
void BCStateTran::fetch() {
  ...
  while (true) {
    ...
    if (newBlockIsValid && isGettingBlocks) {
      ...
      // Metrics set at the end of the block to prevent transaction abort from^M
      // leaving inconsistencies.^M
