...
using PtrToMetaMsgHandler =
    void (ReplicaImp::*)(std::unique_ptr<MessageBase> msg);
...
class ReplicaImp : public InternalReplicaApi, public IReplicaForStateTransfer {
 protected:
  ...
  std::map<ReplicaId, std::unique_ptr<CheckpointMsg>> tableOfStableCheckpoints;
  ...
  typedef concordMetrics::Component::Handle<concordMetrics::Status>
      StatusHandle;
  typedef concordMetrics::Component::Handle<concordMetrics::Counter>
      CounterHandle;
 ...
 public:
  ...
  // InternalReplicaApi
  virtual void onInternalMsg(std::unique_ptr<FullCommitProofMsg> m) override;
 ...
 protected:
  ...
  template <typename T>
  void metaMessageHandler(std::unique_ptr<MessageBase> msg);
  // TODO(GG): rename
  template <typename T>
  ...
  bool tryToEnterView();
  void onNewView(
      const std::vector<std::unique_ptr<PrePrepareMsg>>& prePreparesForNewView);
  ...
  void onMessage(std::unique_ptr<ClientRequestMsg>);
  void onMessage(std::unique_ptr<PrePrepareMsg>);
  void onMessage(std::unique_ptr<StartSlowCommitMsg>);
  void onMessage(std::unique_ptr<PartialCommitProofMsg>);
  void onMessage(std::unique_ptr<FullCommitProofMsg>);
  void onMessage(std::unique_ptr<PartialExecProofMsg>);
  void onMessage(std::unique_ptr<PreparePartialMsg>);
  void onMessage(std::unique_ptr<CommitPartialMsg>);
  void onMessage(std::unique_ptr<PrepareFullMsg>);
  void onMessage(std::unique_ptr<CommitFullMsg>);
  void onMessage(std::unique_ptr<CheckpointMsg>);
  void onMessage(std::unique_ptr<ViewChangeMsg>);
  void onMessage(std::unique_ptr<NewViewMsg>);
  void onMessage(std::unique_ptr<ReqMissingDataMsg>);
  void onMessage(std::unique_ptr<SimpleAckMsg>);
  void onMessage(std::unique_ptr<ReplicaStatusMsg>);
  void onMessage(std::unique_ptr<StateTransferMsg>);
  ...
  void executeReadOnlyRequest(std::unique_ptr<ClientRequestMsg> m);

  void executeReadWriteRequests(const bool requestMissingInfo = false);

  void executeRequestsInPrePrepareMsg(std::unique_ptr<PrePrepareMsg> pp);
