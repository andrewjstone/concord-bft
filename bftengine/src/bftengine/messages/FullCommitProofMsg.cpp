...
FullCommitProofMsg::FullCommitProofMsg(ReplicaId senderId,
  ...
  FullCommitProofMsgHeader* header =
      reinterpret_cast<FullCommitProofMsgHeader*>(mut_data());
  header->viewNum = v;
  header->seqNum = s;
  header->thresholSignatureLength = commitProofSigLength;
  memcpy(mut_data() + sizeof(FullCommitProofMsgHeader),
         commitProofSig,
...
bool FullCommitProofMsg::ToActualMsgType(const ReplicasInfo& repInfo,
                                         ...
                                         std::unique_ptr<FullCommitProofMsg>& outMsg) {
