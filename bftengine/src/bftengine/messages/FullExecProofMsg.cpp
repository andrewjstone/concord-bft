...
FullExecProofMsg::FullExecProofMsg(ReplicaId senderId,
  ...
  memcpy(mut_data() + sizeof(FullExecProofMsgHeader), root, rootLength);
  memcpy(mut_data() + sizeof(FullExecProofMsgHeader) + rootLength,
         executionProof,
         proofLength);
}
...
FullExecProofMsg::FullExecProofMsg(ReplicaId senderId,
  ...
  memcpy(mut_data() + sizeof(FullExecProofMsgHeader) + rootLength,
         readProof,
         proofLength);
  memcpy(mut_data() + sizeof(FullExecProofMsgHeader) + rootLength + proofLength,
         sig,
         sigLength);
}
...
void FullExecProofMsg::setSignature(const char* sig, uint16_t sigLength) {
  ...
  memcpy(mut_data() + sizeof(FullExecProofMsgHeader) + b()->merkleRootLength +
...
bool FullExecProofMsg::ToActualMsgType(NodeIdType myId,
                                       ...
                                       std::unique_ptr<FullExecProofMsg>& outMsg) {
