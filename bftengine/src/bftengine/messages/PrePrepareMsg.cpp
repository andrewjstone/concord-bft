...
std::unique_ptr<PrePrepareMsg> PrePrepareMsg::createNullPrePrepareMsg(ReplicaId sender,
                                                      ViewNum v,
                                                      SeqNum s,
                                                      CommitPath firstPath) {
  std::unique_ptr<PrePrepareMsg> p(new PrePrepareMsg(sender, v, s, firstPath, true));
  return p;
}
...
bool PrePrepareMsg::ToActualMsgType(const ReplicasInfo& repInfo,
                                    ...
                                    std::unique_ptr<PrePrepareMsg>& outMsg) {
  ...
  const PrePrepareMsg* tmp = (PrePrepareMsg*)inMsg;
...
PrePrepareMsg::PrePrepareMsg(
    ...
    : MessageBase(
          sender,
          MsgCode::PrePrepare,
          (isNull ? sizeof(PrePrepareMsgHeader) : maxSizeOfPrePrepareMsg()))
...
void PrePrepareMsg::addRequest(const char* pRequest, uint32_t requestSize) {
  ...
  char* insertPtr = mut_data() + b()->endLocationOfLastRequest;
...
void PrePrepareMsg::finishAddingRequests() {
  ...
  resize(b()->endLocationOfLastRequest);
  shrinkToFit();
}
