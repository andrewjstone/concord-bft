...
void NewViewMsg::addElement(ReplicaId replicaId, Digest& viewChangeDigest) {
  ...
  const NewViewElement* elementsArray =
...
bool NewViewMsg::ToActualMsgType(const ReplicasInfo& repInfo,
                                 std::unique_ptr<inMsg>&,
                                 std::unique_ptr<NewViewMsg>& outMsg) {
...
MsgSize NewViewMsg::maxSizeOfNewViewMsg() { return maxExternalMessageSize; }
