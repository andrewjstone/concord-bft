	...
	namespace impl
	{
                ...
                void ReplicaImp::metaMessageHandler(std::unique_ptr<MessageBase> msg)
		{
                  ...
                  if (isValid) {
                    ...
                    onReportAboutInvalidMessage(msg.get());
                  }	
		}
		...
		void ReplicaImp::onReportAboutInvalidMessage(const MessageBase* msg)
		{
		...
		void ReplicaImp::send(const MessageBase* m, NodeIdType dest)
		{
			// debug code begin 

			if (m->type() == MsgCode::Checkpoint)
			{
				const CheckpointMsg* c = (const CheckpointMsg*)m;
			...
			:  incomingMsgs{ queue }
		{
                ...
                void ReplicaImp::MsgReceiver::onNewMessage(
                  ...
                  incomingMsgs.pushExternalMsg(std::move(m));
		}
		...
		void ReplicaImp::onMessage(std::unique_ptr<ClientRequestMsg> m)
		{
			...
			if (invalidClient || sentFromReplicaToNonPrimary) // TODO(GG): more conditions (make sure that a request of client A cannot be generated by client B!=A)
			{
				...
				onReportAboutInvalidMessage(m.get());
				return;
			}

			if (readOnly)
			{
				executeReadOnlyRequest(std::move(m));
				return;
			}
			...
			if (seqNumberOfLastReply < reqSeqNum)
			{
				...
				else // not the current primary
				{
					if (clientsManager->noPendingAndRequestCanBecomePending(clientId, reqSeqNum))
					{
						...
						send(m.get(), currentPrimary()); // TODO(GG): add a mechanism that retransmits (otherwise we may start unnecessary view-change )
		...
		void ReplicaImp::tryToSendPrePrepareMsg(bool batchingLogic)
		{
			...
			ClientRequestMsg* first = requestsQueueOfPrimary.front().get();
			while (first != nullptr && !clientsManager->noPendingAndRequestCanBecomePending(first->clientProxyId(), first->requestSeqNum()))
			{
				...
				first = (!requestsQueueOfPrimary.empty() ? requestsQueueOfPrimary.front().get() : nullptr);
			}
			...
			controller->onSendingPrePrepare(primaryLastUsedSeqNum, firstPath);
                        ...
                        std::unique_ptr<PrePrepareMsg> pp(new PrePrepareMsg(myReplicaId, curView, primaryLastUsedSeqNum, firstPath, false));

			ClientRequestMsg* nextRequest = requestsQueueOfPrimary.front().get();
			while (nextRequest != nullptr && nextRequest->size() <= pp->remainingSizeForRequests())
			{
				...
				requestsQueueOfPrimary.pop();
				nextRequest = (requestsQueueOfPrimary.size() > 0 ? requestsQueueOfPrimary.front().get() : nullptr);
			}
			...
			{
				sendRetransmittableMsgToReplica(pp.get(), x, primaryLastUsedSeqNum);
			}
		...
		template <typename T>
		bool ReplicaImp::relevantMsgForActiveView(const std::unique_ptr<T>& msg)
		{
		...
		void ReplicaImp::onMessage(std::unique_ptr<PrePrepareMsg> msg)
		{
			...
			if (!currentViewIsActive() && viewsManager->waitingForMsgs() && msgSeqNum > lastStableSeqNum)
			{
				...
				if (viewsManager->addPotentiallyMissingPP(std::move(msg), lastStableSeqNum))
				{
			...
			{
				sendAckIfNeeded(msg.get(), msg->senderId(), msgSeqNum);

				SeqNumInfo& seqNumInfo = mainLog->get(msgSeqNum);

                                auto first_path = msg->firstPath();
				if (seqNumInfo.addMsg(std::move(msg)))
				{
					if (first_path != CommitPath::SLOW && !seqNumInfo.slowPathStarted()) // TODO(GG): make sure we correctly handle a situation where StartSlowCommitMsg is handled before PrePrepareMsg
					{
						sendPartialProof(seqNumInfo);
					}
					else
		...
		void ReplicaImp::tryToStartSlowPaths()
		{
			...
			for (SeqNum i = minSeqNum; i <= maxSeqNum; i++)
			{
				...
				// send StartSlowCommitMsg to all replicas
                                ...
                                std::unique_ptr<StartSlowCommitMsg> startSlow(new StartSlowCommitMsg(myReplicaId, curView, i));

				for (ReplicaId x : repsInfo->idsOfPeerReplicas())
				{
					sendRetransmittableMsgToReplica(startSlow.get(), x, i);
				}
		...
		void ReplicaImp::onMessage(std::unique_ptr<StartSlowCommitMsg> msg)
		{
			...
			{
				sendAckIfNeeded(msg.get(), currentPrimary(), msgSeqNum);
		...
		void ReplicaImp::sendPartialProof(SeqNumInfo& seqNumInfo)
		{
			...
			const PrePrepareMsg* pp = seqNumInfo.getPrePrepareMsg();
			...
			if (!partialProofs.hasFullProof())
			{
				...
				if (part == nullptr)
				{
					...
					Digest::calcCombination(ppDigest, curView, seqNum, tmpDigest);
                                        ...
                                        std::unique_ptr<PartialCommitProofMsg> p(new PartialCommitProofMsg(myReplicaId, curView, seqNum, commitPath, tmpDigest, commitSigner));
					partialProofs.addSelfMsgAndPPDigest(part, tmpDigest);
                                        part = partialProofs.getSelfPartialCommitProof();
				}
		...
		void ReplicaImp::sendPreparePartial(SeqNumInfo& seqNumInfo)
		{
			...
			if (seqNumInfo.getSelfPreparePartialMsg() == nullptr && seqNumInfo.hasPrePrepareMsg() && !seqNumInfo.isPrepared())
			{
				...
				auto p = PreparePartialMsg::create(curView, pp->seqNumber(), myReplicaId, pp->digestOfRequests(), thresholdSignerForSlowPathCommit);
				seqNumInfo.addSelfMsg(std::move(p));

				if (!isCurrentPrimary()) {
                                  sendRetransmittableMsgToReplica(p.get(), currentPrimary(), pp->seqNumber());
                                }
			}
		}
		...
		void ReplicaImp::onMessage(std::unique_ptr<PartialCommitProofMsg> msg)
		{
			...
			if (relevantMsgForActiveView(msg))
			{
				sendAckIfNeeded(msg.get(), msgSender, msgSeqNum);
		...
		void ReplicaImp::onMessage(std::unique_ptr<CheckpointMsg> msg)
		{
			...
			if (msgIsStable && msgSeqNum > lastExecutedSeqNum)
			{
				...
				{
                                        std::unique_ptr<CheckpointMsg> x(new CheckpointMsg(msgSenderId, msgSeqNum, msgDigest, msgIsStable));
		...
		void ReplicaImp::sendAckIfNeeded(const MessageBase* msg, const NodeIdType sourceNode, const SeqNum seqNum)
		{
			...
			{
                          std::unique_ptr<SimpleAckMsg> ackMsg(new SimpleAckMsg(seqNum, curView, myReplicaId, msg->type()));
                          send(ackMsg.get(), sourceNode);
			}
		}


		void ReplicaImp::sendRetransmittableMsgToReplica(const MessageBase* msg, ReplicaId destReplica, SeqNum s, bool ignorePreviousAcks)
		{

			send(msg.get(), destReplica);
			...
			const std::forward_list<RetSuggestion>* const suggestedRetransmissions)
		{
			...
			for (const RetSuggestion& s : *suggestedRetransmissions)
			{
				...
				*/
				...
				{
                                        std::unique_ptr<StartSlowCommitMsg> msgToSend(StartSlowCommitMsg(myId, curView, s.msgSeqNum));
					sendRetransmittableMsgToReplica(msgToSend, s.replicaId, s.msgSeqNum);
		...
		void ReplicaImp::onMessage(std::unique_ptr<ReplicaStatusMsg> msg)
		{
			...
			{
				const CheckpointMsg* checkMsg = checkpointsLog->get(lastStableSeqNum).selfCheckpointMsg();
...
//				const SeqNum prevLastExecutedSeqNum = lastExecutedSeqNum;
				...
				stateTransfer->getDigestOfCheckpoint(checkpointNum, sizeof(Digest), (char*)&digestOfNewState);
                                std::unique_ptr<CheckpointMsg> checkpointMsg(new CheckpointMsg(myReplicaId, newStateCheckpoint, digestOfNewState, false));
		...
		void ReplicaImp::onSeqNumIsStable(SeqNum newStableSeqNum)
		{
			...
			if (checkpointMsg == nullptr)
			{
				...
				stateTransfer->getDigestOfCheckpoint(checkpointNum, sizeof(Digest), (char*)&digestOfState);
                                std::unique_ptr<CheckpointMsg> c(new CheckpointMsg(myReplicaId, lastStableSeqNum, digestOfState, true));
				checkpointInfo.addCheckpointMsg(c, myReplicaId);
                                checkpointMsg = checkpointInfo.selfCheckpointMsg();
			}
			else
			{
		...
		void ReplicaImp::onMessage(std::unique_ptr<ReqMissingDataMsg> msg)
		{
				...
				(mainLog->insideActiveWindow(msgSeqNum)) && (mainLog->insideActiveWindow(msgSeqNum)))
			{
				...
				{
					const PrePrepareMsg* pp = seqNumInfo.getSelfPrePrepareMsg();
...
//			initAllocator();
		...
		void ReplicaImp::executeReadOnlyRequest(std::unique_ptr<ClientRequestMsg> request) 
		{
		...
		void ReplicaImp::executeRequestsInPrePrepareMsg(PrePrepareMsg* ppMsg)
		{
			...
			while (reqIter.getAndGoToNext(requestBody)) 
			{
				...
				{	
                                  std::unique_ptr<ClientReplyMsg> replyMsg = clientsManager->allocateMsgWithLatestReply(clientId, currentPrimary());
				...
				Assert(actualReplyLength > 0); // TODO(GG): TBD - how do we want to support empty replies? (actualReplyLength==0)
                                ...
                                std::unique_ptr<ClientReplyMsg> replyMsg = clientsManager->allocateNewReplyMsgAndWriteToStorage(clientId, req.requestSeqNum(), currentPrimary(), replyBuffer, actualReplyLength);
