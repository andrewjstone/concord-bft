...
class FullExecProofMsg : public MessageBase {
 public:
  FullExecProofMsg(ReplicaId senderId, NodeIdType clientId, ReqId requestId,
                   uint16_t sigLength, const char* root, uint16_t rootLength,
                   const char* executionProof, uint16_t proofLength);

  FullExecProofMsg(ReplicaId senderId, NodeIdType clientId, const char* sig,
                   uint16_t sigLength, const char* root, uint16_t rootLength,
  ...
  static bool ToActualMsgType(NodeIdType myId, MessageBase* inMsg,
