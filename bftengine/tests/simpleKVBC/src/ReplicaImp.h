...
namespace SimpleKVBC {
	...
	protected:
		...
		Sliver getBlockInternal(BlockId blockId) const;
                concord::storage::blockchain::BlockchainDBAdapter* getBcDbAdapter() const { return m_bcDbAdapter; }
		...
		StorageWrapperForIdleMode m_InternalStorageWrapperForIdleMode;
                ...
                concord::storage::blockchain::BlockchainDBAdapter* m_bcDbAdapter;
		BlockId lastBlock = 0;
		...
		friend IReplica* createReplica(
                    const bftEngine::ReplicaConfig& conf,
