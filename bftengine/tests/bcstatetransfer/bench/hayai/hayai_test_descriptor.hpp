...
class TestParameterDescriptor {
 public:
  TestParameterDescriptor(std::string declaration, std::string value) : Declaration(declaration), Value(value) {}
...
class TestParametersDescriptor {
 private:
  ...
  TestParameterDescriptor ParseDescriptor(const std::string& raw) {
    ...
    if (equalPosition) {
      ...
      return TestParameterDescriptor(std::string(TrimmedString(start, equalPosition - 1)),
                                     std::string(TrimmedString(equalPosition, end)));
    } else
 ...
 public:
  ...
  TestParametersDescriptor(const char* rawDeclarations, const char* rawValues) {
    ...
    std::vector<std::string> declarations = ParseCommaSeparated(rawDeclarations);

    for (std::vector<std::string>::const_iterator it = declarations.begin(); it != declarations.end(); ++it)
    ...
    if (values.size() > _parameters.size()) {
      if (straightValues > 0) --straightValues;
      variadicValues = values.size() - _parameters.size() + 1;
    }

    for (std::size_t i = 0; i < straightValues; ++i) _parameters[i].Value = values[i];
  ...
  inline const std::vector<TestParameterDescriptor>& Parameters() const { return _parameters; }
