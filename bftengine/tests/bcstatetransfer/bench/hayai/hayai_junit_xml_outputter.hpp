...
class JUnitXmlOutputter : public Outputter {
   ...
   public:
    TestCase(const std::string& fixtureName,
      ...
      if (result) {
        ...
        timeStream << std::fixed << std::setprecision(9) << (result->IterationTimeAverage() / 1e9);
        Time = timeStream.str();
      }
    }
 ...
 public:
  ...
  /// the outputter's use.
  JUnitXmlOutputter(std::ostream& stream) : _stream(stream) {}
  ...
  virtual void End(const std::size_t& executedCount, const std::size_t& disabledCount) {
    ...
    for (TestSuiteMap::iterator testSuiteIt = _testSuites.begin(); testSuiteIt != _testSuites.end(); ++testSuiteIt) {
      ...
      for (std::vector<TestCase>::iterator testCaseIt = testSuiteIt->second.begin();
        ...
        if (!testCaseIt->Skipped)
          ...
          _stream << ">" << std::endl << "            <skipped />" << std::endl << "        </testcase>" << std::endl;
        }
      }
  ...
  virtual void EndTest(const std::string& fixtureName,
    ...
    TestSuiteMap::iterator fixtureIt = _testSuites.find(fixtureName);
    ...
    testCases.push_back(TestCase(fixtureName, testName, parameters, &result));
