...
class JsonOutputter : public Outputter {
 public:
  ...
  /// the outputter's use.
  JsonOutputter(std::ostream& stream) : _stream(stream), _firstTest(true) {}
  ...
  virtual void End(const std::size_t& executedCount, const std::size_t& disabledCount) {
    ...
    _stream << JSON_ARRAY_END JSON_OBJECT_END;
  }
  ...
  virtual void BeginTest(const std::string& fixtureName,
    ...
    BeginTestObject(fixtureName, testName, parameters, runsCount, iterationsCount, false);
  }
  ...
  virtual void SkipDisabledTest(const std::string& fixtureName,
                                ...
                                const std::size_t& runsCount,
                                const std::size_t& iterationsCount) {
    BeginTestObject(fixtureName, testName, parameters, runsCount, iterationsCount, true);
    EndTestObject();
  }
  ...
  virtual void EndTest(const std::string& fixtureName,
    ...
    for (std::vector<uint64_t>::const_iterator it = runTimes.begin(); it != runTimes.end(); ++it) {
      if (it != runTimes.begin()) _stream << JSON_VALUE_SEPARATOR;

      _stream << JSON_OBJECT_BEGIN JSON_STRING_BEGIN "duration" JSON_STRING_END JSON_NAME_SEPARATOR << std::fixed
              << std::setprecision(6) << (double(*it) / 1000000.0) << JSON_OBJECT_END;
    }
 ...
 private:
  void BeginTestObject(const std::string& fixtureName,
    ...
    if (!descs.empty()) {
      ...
      for (std::size_t i = 0; i < descs.size(); ++i) {
        if (i) _stream << JSON_VALUE_SEPARATOR;

        const TestParameterDescriptor& desc = descs[i];

        _stream << JSON_OBJECT_BEGIN

                JSON_STRING_BEGIN "declaration" JSON_STRING_END JSON_NAME_SEPARATOR;

        WriteString(desc.Declaration);

        _stream << JSON_VALUE_SEPARATOR

                JSON_STRING_BEGIN "value" JSON_STRING_END JSON_NAME_SEPARATOR;
    ...
    _stream << JSON_STRING_BEGIN "iterations_per_run" JSON_STRING_END JSON_NAME_SEPARATOR << iterationsCount <<

        JSON_VALUE_SEPARATOR

            JSON_STRING_BEGIN "disabled" JSON_STRING_END JSON_NAME_SEPARATOR
            << (disabled ? JSON_TRUE : JSON_FALSE);
  }

  inline void EndTestObject() { _stream << JSON_OBJECT_END; }
  ...
  void WriteDoubleProperty(const std::string& key, const double value) {
    _stream << JSON_VALUE_SEPARATOR << JSON_STRING_BEGIN << key << JSON_STRING_END << JSON_NAME_SEPARATOR << std::fixed
            << std::setprecision(6) << (value / 1000000.0);
  }
