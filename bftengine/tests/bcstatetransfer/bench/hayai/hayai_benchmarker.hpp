...
class Benchmarker {
 public:
  ...
  static TestDescriptor* RegisterTest(const char* fixtureName,
    ...
    if (isDisabled) testName += 9;

    // Add the descriptor.
    TestDescriptor* descriptor =
        new TestDescriptor(fixtureName, testName, runs, iterations, testFactory, parameters, isDisabled);
  ...
  static void AddOutputter(Outputter& outputter) { Instance()._outputters.push_back(&outputter); }
  ...
  static void ApplyPatternFilter(const char* pattern) {
    ...
    while (index < instance._tests.size()) {
      ...
      if ((!FilterMatchesString(positive.c_str(), desc->CanonicalName)) ||
        ...
        instance._tests.erase(instance._tests.begin() + std::vector<TestDescriptor*>::difference_type(index));
        delete desc;
      } else
        ++index;
    }
  ...
  static void RunAllTests() {
    ...
    Benchmarker& instance = Instance();
    std::vector<Outputter*>& outputters = (instance._outputters.empty() ? defaultOutputters : instance._outputters);
    ...
    std::vector<TestDescriptor*>::const_iterator testsIt = tests.begin();

    while (testsIt != tests.end()) {
      if ((*testsIt)->IsDisabled) ++disabledCount;
      ++testsIt;
    }
    ...
    // Begin output.
    for (std::size_t outputterIndex = 0; outputterIndex < outputters.size(); outputterIndex++)
    ...
    while (index < tests.size()) {
      ...
      for (std::size_t outputterIndex = 0; outputterIndex < outputters.size(); outputterIndex++)
        outputters[outputterIndex]->BeginTest(descriptor->FixtureName,
                                              descriptor->TestName,
                                              descriptor->Parameters,
                                              descriptor->Runs,
                                              descriptor->Iterations);
      ...
      uint64_t overheadCalibration = calibrationModel.GetCalibration(descriptor->Iterations);
      ...
      while (run < descriptor->Runs) {
        ...
        // Store the test time.
        runTimes[run] = (time > overheadCalibration ? time - overheadCalibration : 0);
      ...
      for (std::size_t outputterIndex = 0; outputterIndex < outputters.size(); outputterIndex++)
        outputters[outputterIndex]->EndTest(
            descriptor->FixtureName, descriptor->TestName, descriptor->Parameters, testResult);
    }
  ...
  static std::vector<const TestDescriptor*> ListTests() {
    ...
    std::size_t index = 0;
    while (index < instance._tests.size()) tests.push_back(instance._tests[index++]);
  ...
  static void ShuffleTests() {
    ...
    std::mt19937 g(rd());
    std::shuffle(instance._tests.begin(), instance._tests.end(), g);
#else
    std::random_shuffle(instance._tests.begin(), instance._tests.end());
#endif
  }
 ...
 private:
  ...
  struct CalibrationModel {
   public:
    CalibrationModel(std::size_t scale, uint64_t slope, uint64_t yIntercept)
        : Scale(scale), Slope(slope), YIntercept(yIntercept) {}
    ...
    int64_t GetCalibration(std::size_t iterations) const { return YIntercept + (iterations * Slope) / Scale; }
  };
  ...
  ~Benchmarker() {
    ...
    while (index--) delete _tests[index];
  }
  ...
  std::vector<TestDescriptor*> GetTests() const {
    ...
    std::size_t index = 0;
    while (index < _tests.size()) tests.push_back(_tests[index++]);
  ...
  static bool FilterMatchesString(const char* filter, const std::string& str) {
    ...
    while (true) {
      if (PatternMatchesString(patternStart, str.c_str())) return true;

      // Finds the next pattern in the filter.
      patternStart = strchr(patternStart, ':');

      // Returns if no more pattern can be found.
      if (!patternStart) return false;
...
#define HAYAI_CALIBRATION_PPR 6
    ...
    uint64_t interceptSum = 0, interceptMin = std::numeric_limits<uint64_t>::min(), interceptMax = 0;

    for (std::size_t run = 0; run < HAYAI_CALIBRATION_INTERESECT_RUNS; ++run) {
      uint64_t intercept = test->Run(0);
      interceptSum += intercept;
      if (intercept < interceptMin) interceptMin = intercept;
      if (intercept > interceptMax) interceptMax = intercept;
    }

    uint64_t interceptAvg = interceptSum / HAYAI_CALIBRATION_INTERESECT_RUNS;

    // Produce a series of sample points.
    std::vector<uint64_t> x(HAYAI_CALIBRATION_RUNS * HAYAI_CALIBRATION_PPR);
    std::vector<uint64_t> t(HAYAI_CALIBRATION_RUNS * HAYAI_CALIBRATION_PPR);
...
#define HAYAI_CALIBRATION_POINT(_x) \
  x[point] = _x;                    \
  t[point++] = test->Run(_x * std::size_t(HAYAI_CALIBRATION_SCALE))
...
#undef HAYAI_CALIBRATION_POINT
    ...
    //  {\sum_{i=1}^{n} {x_n}^2}$.
    uint64_t sumProducts = 0, sumXSquared = 0;
    ...
    return CalibrationModel(HAYAI_CALIBRATION_SCALE, slope, interceptAvg);
