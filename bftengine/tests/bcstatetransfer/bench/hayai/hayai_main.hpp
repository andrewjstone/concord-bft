...
#define HAYAI_MAIN_USAGE_ERROR(_desc)                                      \
  {                                                                        \
    std::cerr << HAYAI_MAIN_FORMAT_ERROR(_desc) << std::endl << std::endl; \
    ShowUsage(argv[0]);                                                    \
    return EXIT_FAILURE;                                                   \
  }
...
class FileOutputter {
 public:
  ...
  FileOutputter(const char* path) : _path(path), _outputter(NULL) {}

  virtual ~FileOutputter() {
    if (_outputter) delete _outputter;
  ...
  virtual ::hayai::Outputter& Outputter() {
    if (!_outputter) throw std::runtime_error("outputter has not been set up");
...
#define FILE_OUTPUTTER_IMPLEMENTATION(_prefix)                          \
  class _prefix##FileOutputter : public FileOutputter {                 \
   public:                                                              \
    _prefix##FileOutputter(const char* path) : FileOutputter(path) {}   \
                                                                        \
   protected:                                                           \
...
class MainRunner {
 public:
  MainRunner() : ExecutionMode(MainRunBenchmarks), ShuffleBenchmarks(false), StdoutOutputter(NULL) {}

  ~MainRunner() {
    // Clean up the outputters.
    for (std::vector<FileOutputter*>::iterator it = FileOutputters.begin(); it != FileOutputters.end(); ++it)
      delete *it;

    if (StdoutOutputter) delete StdoutOutputter;
  }
  ...
  int ParseArgs(int argc, char** argv, std::vector<char*>* residualArgs = NULL) {
    ...
    while (argI < argc) {
      ...
      // List flag.
      if ((!strcmp(arg, "-l")) || (!strcmp(arg, "--list"))) ExecutionMode = ::hayai::MainListBenchmarks;
      // Shuffle flag.
      ...
      // Filter flag.
        ...
        if ((argLast) || (*argv[argI] == 0))
          HAYAI_MAIN_USAGE_ERROR(HAYAI_MAIN_FORMAT_FLAG(arg) << " requires a pattern to be specified");
      ...
      // Output flag.
        ...
        if (argLast) HAYAI_MAIN_USAGE_ERROR(HAYAI_MAIN_FORMAT_FLAG(arg) << " requires a format to be specified");
...
#define ADD_OUTPUTTER(_prefix)                                             \
  {                                                                        \
    if (path)                                                              \
      FileOutputters.push_back(new ::hayai::_prefix##FileOutputter(path)); \
    else {                                                                 \
      if (StdoutOutputter) delete StdoutOutputter;                         \
      StdoutOutputter = new ::hayai::_prefix##Outputter(std::cout);        \
    }                                                                      \
  }
...
#undef ADD_OUTPUTTER
      ...
      // Console coloring flag.
        ...
        if (argLast)
          HAYAI_MAIN_USAGE_ERROR(HAYAI_MAIN_FORMAT_FLAG(arg) << " requires an argument "
        ...
        if ((!strcmp(choice, "yes")) || (!strcmp(choice, "true")) || (!strcmp(choice, "on")) || (!strcmp(choice, "1")))
          enabled = true;
        else if ((!strcmp(choice, "no")) || (!strcmp(choice, "false")) || (!strcmp(choice, "off")) ||
                 (!strcmp(choice, "0")))
          enabled = false;
        else
          HAYAI_MAIN_USAGE_ERROR("invalid argument to " << HAYAI_MAIN_FORMAT_FLAG(arg) << ": " << choice);
  ...
  int Run() {
    ...
    switch (ExecutionMode) {
      ...
      default:
        std::cerr << HAYAI_MAIN_FORMAT_ERROR("invalid execution mode: " << ExecutionMode) << std::endl;
        return EXIT_FAILURE;
    }
  }
 ...
 private:
  ...
  int RunBenchmarks() {
    // Hook up the outputs.
    if (StdoutOutputter) ::hayai::Benchmarker::AddOutputter(*StdoutOutputter);

    for (std::vector< ::hayai::FileOutputter*>::iterator it = FileOutputters.begin(); it < FileOutputters.end(); ++it) {
  ...
  int ListBenchmarks() {
    ...
    std::vector<const ::hayai::TestDescriptor*> tests = ::hayai::Benchmarker::ListTests();
    std::vector<std::string> testNames;
    std::set<std::string> uniqueTestNames;

    for (std::vector<const ::hayai::TestDescriptor*>::iterator it = tests.begin(); it < tests.end(); ++it) {
      if (uniqueTestNames.find((*it)->CanonicalName) != uniqueTestNames.end()) continue;
  ...
  void ShowUsage(const char* execName) {
    ...
    std::cerr << "Usage: " << (baseName ? baseName + 1 : execName) << " " << HAYAI_MAIN_FORMAT_FLAG("[OPTIONS]")
              << std::endl
              << std::endl
              ...
              << "  " << HAYAI_MAIN_FORMAT_FLAG("-s") << ", " << HAYAI_MAIN_FORMAT_FLAG("--shuffle") << std::endl
              ...
              << "    in different formats. The supported formats are:" << std::endl
              << std::endl
              << "    " << HAYAI_MAIN_FORMAT_ARGUMENT("console") << std::endl
              << "      Standard console output." << std::endl
              << "    " << HAYAI_MAIN_FORMAT_ARGUMENT("json") << std::endl
              << "      JSON." << std::endl
              << "    " << HAYAI_MAIN_FORMAT_ARGUMENT("junit") << std::endl
              ...
              << ::hayai::Console::TextGreen << "yes" << ::hayai::Console::TextDefault << "|"
              << ::hayai::Console::TextGreen << "no" << ::hayai::Console::TextDefault << ")" << std::endl
              << "    Enable colored output when available. Default " << ::hayai::Console::TextGreen << "yes"
              ...
              << "Clock implementation: " << ::hayai::Clock::Description() << std::endl;
  }
};
}  // namespace hayai
