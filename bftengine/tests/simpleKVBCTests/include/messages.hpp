...
#include "storage/blockchain_db_interfaces.h"
...
struct SimpleBlock {
  concord::storage::blockchain::BlockId id = 0;
  size_t numOfItems = 0;
  SimpleKV items[1];
...
// A SimpleGetBlockData returns a read response, except
...
struct SimpleGetBlockDataRequest {
  ...
  static void free(SimpleGetBlockDataRequest* p) {
    char* p1 = (char*)p;
    delete[] p1;
  }
